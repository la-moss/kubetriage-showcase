...
This document describes the optional pipeline for generating replay fixtures from a local kind cluster. The pipeline is local-only and not required for KubeClaw diagnosis, evaluation, or the sample outputs in this showcase.

Normal KubeClaw tests, demos, replay, and evaluation remain fully offline and do not require kind, kubectl, Docker, a live cluster, or network access.

## Pipeline overview

```
1. Create dedicated kind cluster (name: kind-kubeclaw)
      ↓
...
The local kind lab creates a dedicated `kind-kubeclaw` cluster and deploys controlled broken workloads:
...
All workloads run in a dedicated `kubeclaw-lab` namespace.
...
**Capture is not diagnosis.** Generated sessions are unreviewed and are **not** automatically golden or holdout material. **Replay evidence remains the source of truth** for KubeClaw evaluation.
...
- **Replay evidence remains source of truth.** KubeClaw's correctness is evaluated against deterministic replay, not live cluster behaviour.
- **No remediation is executed by KubeClaw.** The lab deploys broken workloads; KubeClaw reads evidence. No write operations are performed by the engine.
- **Lab setup uses controlled manifests only.** Scenario deployment targets fixed lab manifests — this is not KubeClaw diagnosis or remediation.
